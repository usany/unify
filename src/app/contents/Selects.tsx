  ...
  return (
    <div className={`flex ${matches ? "" : "flex-col"} gap-1 px-5`}>
      <FormControl variant="standard" sx={{ width: 150 }}>
        <InputLabel>
          {language === 'ko' ? '위치가 어디인가요' : 'Which location?'}
        </InputLabel>
        <Select
        ...
        >
          {locationsBuildingsArray.map((value, index) => {
            ...
            const campuses = language === 'ko' ? ['서울', '국제', '광릉'] : ['Seoul', 'Global', 'Gwangneung']
            const campusIndex = key.slice(0, 2) === 'se' ? 0 : key.slice(0, 2) === 'gu' ? 1 : 2
            const nameWithoutCampus = campuses.indexOf(value[key][language].name.split(' ')[0]) === -1 ? value[key][language].name : value[key][language].name.replace(value[key][language].name.split(' ')[0], '')
      ...
      {locationState.locationOne !== "" &&
        locationState.locationOne !== "직접 입력" && (
          <FormControl variant="standard" sx={{ width: 150 }}>
            ...
            >
              {language === 'en' && 'Where in '}{language === 'ko' ? locationState.locationOne : locationsBuildings['en'][locationsBuildings['ko'].indexOf(locationState.locationOne)]}{language === 'ko' && ' 어디인가요'}
            </InputLabel>
            <Select
            ...
            >
              {menuItems}
              {/* {locationState.locationOne === "중도" && location.cl[language]}
              {locationState.locationOne === "청운" && location.cw[language]}
              {locationState.locationOne === "푸른솔" && location.p[language]}
              {locationState.locationOne === "간호이과대" && location.g[language]}
              {locationState.locationOne === "네오르네상스관" && location.n[language]}
              {locationState.locationOne === "경영대" && location.k[language]}
              {locationState.locationOne === "문과대" && location.m[language]}
              {locationState.locationOne === "의과대" && location.e[language]}
              {locationState.locationOne === "치과병원" && location.c[language]} */}
            </Select>
          </FormControl>
        )}
      {["1열(1F)", "2열(2F)", "3열(2F)", "4열(4F)", "집중열(1F)", '벗터(1F)', '혜움(2F)'].indexOf(
        locationState.locationTwo
          ...
          <FormControl variant="standard" sx={{ width: 150 }}>
            ...
            // id="demo-simple-select-standard-label1"
            >
              {language === 'ko' ? '좌석이 어디인가요' : 'Where is your seat'}
            </InputLabel>
            <Select
              // labelId="demo-simple-select-standard-label1"
              // id="demo-simple-select-standard"
