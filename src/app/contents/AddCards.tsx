...
const AddCards = ({ borrow, item, fromTo, locationState }: Props) => {
  ...
  const key = locationOne ? Object.keys(locationsCollectionLetters).find((value) => {
    ...
    const campusObj = buildingsObj[campusKey] as { [key: string]: { ko: { name: string }, en: { name: string }, image: StaticImageData } }
    return campusObj[value].ko.name === locationOne
  }) : ''
  const staticImg = (locationOne && locationOne !== '직접 입력') ? 
    (key ? (buildingsObj[key.slice(0, 2) as keyof typeof buildingsObj] as { [key: string]: { ko: { name: string }, en: { name: string }, image: StaticImageData } })[key]?.image : buildingsObj.input.image) 
